...
N     = 200;            % Number of Gridpoints in Real Wealth Space
...
steady.Dr        = Dr;
steady.mu_index  = mu_index;
steady.mu_vec    = mu_vec;
steady.M0_ss     = M0_ss;
...
% [5] Money change
M0_t                   = M0_ss*ones(1,T);  
M0_t(T_pre+1:T_post)   = M0_ss*shock_M;
...
% Paths for exogenous 
paths_list={'rr_t','rsp_t','mu_w_t','sigma_w_t','s_bl_t','T_t','M0_t'};
paths_list_tit={'r^o^r_t','r^d^w_t','\mu_2','\sigma_2','s^b^l_t','T_t','M0_t'};
...
% rs_t_o(T_pre+1:T_post) = rs_ss-1/2*rsp_t(T_pre+1:T_post);
...
switch mpregime
    ...
    case {'MP'}
        eqpath=@(rs_t_o) MPCC_path_solve_MP(rs_t_o,init_con,steady,paths);
end
