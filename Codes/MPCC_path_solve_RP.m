function [res,aux,D_t]=MPCC_path_solve_RP(rs_t,init_con,steady,paths)
...
for tt=1:T
    if tt<=index_T_T
        ...
        TE_t(tt+1) = (1+rb_t(tt))*TE_t(tt)-RT_t(tt)   ;
    elseif tt>index_T_T
        aux_weight=delta_trans^(tt-T_post)                    ;
        TE_t_aux(tt+1) = TE_ss+delta_ef*(TE_t(tt)-TE_ss)      ;
        RT_t_aux(tt) = (1+rb_t(tt))*TE_t(tt)-TE_t_aux(tt+1)   ; % Find transfer rule that clears things
