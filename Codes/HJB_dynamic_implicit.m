function [V]=HJB_dynamic_implicit(V,U,s,muF,muB,sigma,rho,dt,Ib,If)
% ------------------------------------------
% USES:
% ------------------------------------------
%
% Solving: r*V(s) - mu(s)*V'(s) - sigma(s)^2*V''(s)/2 - U(s) = V_t;
%
...
% --------------------------------------------------------------------------
% Note, we are assuming no volatility at the right or left ----> code is
% not general yet
% --------------------------------------------------------------------------
% Vectorization
N = length(s)         ; % Length
...
V = (speye(N)+dt*A)\(U*dt+V); % (Yuliy's approach is to apply operator in space to V(t) 
end