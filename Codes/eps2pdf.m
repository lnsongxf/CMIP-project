function [result,msg] = eps2pdf(epsFile, orientation)
% ESP2PDF  Convierte un archivo PS o EPS en PDF utilizando GhostScript
% --------------------------------------------------------------------------------------------------
% USO  : [ resultado, msg ] = eps2pdf(nombre)
% --------------------------------------------------------------------------------------------------
%    nombre    : Nombre del archivo por ser convertido a pdf
%    resultado : 0 si la ejecución no presenta errores, 1 de otro modo
%    msg       : Estatus del archivo procesado
% --------------------------------------------------------------------------------------------------
% Función del Departamento de Modelos Macroeconómicos, BCRP (por Diego Winkelried, Agosto 2010)
% Adaptación de función hecha por Primoz Cermelj (primoz.cermelj@email.si, Octubre 2004)
% --------------------------------------------------------------------------------------------------
...
if nargin < 2
    [result,msg] = eps2pdf0(epsFile, ruta);
else
    [result,msg] = eps2pdf0(epsFile, ruta, orientation);
end

function [result,msg] = eps2pdf0(epsFile,fullGsPath,orientation)
