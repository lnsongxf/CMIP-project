...
fedbal_p   =@(mu) 1/2*max(mu-varrho+omega*(1-varrho),0)+1/2*max(mu-varrho-omega*(1-varrho),0);
fedbal_m   =@(mu) 1/2*max(varrho-mu+omega*(1-varrho),0)+1/2*max(varrho-mu-omega*(1-varrho),0); 

% Compute Market Tightness
...
% mu inverse
dwprofs=@(pol_d,mu)     (chi_m(pol_d,theta(mu)).*fedbal_m(mu)-chi_p(pol_d,theta(mu)).*fedbal_p(mu));
dwprofs_z=@(pol_d,mu)   0;
dwprofs_o=@(pol_d,mu)   (chi_m_o(pol_d).*fedbal_m(mu)-chi_p_o(pol_d).*fedbal_p(mu));
