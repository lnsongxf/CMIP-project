...
N = 250;            % Number of Gridpoints in Real Wealth Space

% Time Parameters
T     = 100      ;  % Time Horizon
Titer = 1        ;  
Delta = 100       ;  % Delta for solving Bellman equation at the stationary equilibrium.
...
if flag_unant
    ...
    T_post= 20       ; % Time Lapse after transition
end
...
rho   = 0.02 ; % agent's individual discount rates 
w1    = 5    ; % mean return - low intensity technology
w2    = 12.5 ; % mean return - high intensity technology
s1    = 0    ; % volatility - low intensity technology
s2    = 8    ; % volatility - high intensity technology

% Initial Guess for interest rate rate
rs_o = 0.01;

% Borrowing Limit - nominal
s_bar      = -3*w2        ;  % debt limit 
...
eta     = 0.5  ; % Bargaining
omega   = 0.15 ; % Average size of shock - interbank
...
% shock
c_dl = rho*s_bar + w1; %w1; % Stays Constant 
