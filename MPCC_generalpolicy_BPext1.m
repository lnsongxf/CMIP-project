...
shock_mu   = 1.0                            ;
shock_sigma= 1.0                            ;
rsp_shock  = 1;%[0.9 0.85 0.75 0.6 0.5 ...
              %0.4 0.4 0.4 0.4 0.4];
shock_s_bl = 0.8;%[0.7*ones(1,8) 0.8 .9];
shock_T    = 0                              ;  
...
RT_ss         = TE_ss*rs            ;
FT_ss         = rsp_ss*B_ss         ;
unemp_ss      = (1-N1/N2)*sum(f_ss(s_bl_index))*ds; % Careful with agents that are short
...
for t=1:T
    ...
    unemp_t(t) = sum(f_t(1:temp,t)*ds)*(1-N1/N2);
end
...
figure(cc)
plot(Periods(1:t_plotmax)-1,unemp_t(1:t_plotmax)*100,'linewidth',4); grid on;
axis tight; [yout]=get(gca,'ylim');
line([T_pre T_pre],[yout(1) yout(2)],'Color','k','LineWidth',2,'LineStyle','--');
line([T_post-1 T_post-1],[yout(1) yout(2)],'Color','k','LineWidth',2,'LineStyle','--');
h=patch([T_pre T_post-1 T_post-1 T_pre],[yout(1) yout(1) yout(2) yout(2)],[0.9 0.9 0.9]); alpha(h,0.2);
